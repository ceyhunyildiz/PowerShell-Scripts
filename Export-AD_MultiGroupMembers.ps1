...
# Kullanıcıları tekilleştirmek ve hangi gruplardan geldiğini tutmak için
# Key: SamAccountName
# Value: PSCustomObject (UserInfo + Üye Olduğu Gruplar)
$userMap = @{}
...
foreach ($groupInput in $groupInputs) {
    ...
    foreach ($u in $userMembers) {
        try {
            ...
            if ($adUser.Manager) {
                try {
                    $managerName = (Get-ADUser -Identity $adUser.Manager -Properties DisplayName -ErrorAction Stop).DisplayName
                } catch {
            ...
            # Daha önce eklendiyse sadece grup listesine ekle
            if ($userMap.ContainsKey($key)) {
                $existingGroups = $userMap[$key]."Üye Olduğu Gruplar"
                $groupList = $existingGroups -split '\s*;\s*' | Where-Object { $_ }

                if ($groupList -notcontains $group.Name) {
                    $groupList += $group.Name
                    $userMap[$key]."Üye Olduğu Gruplar" = ($groupList | Sort-Object) -join "; "
                }
            }
            else {
                # İlk kez görüyorsak kullanıcı kaydını oluştur
...
if (-not $usersDetailed -or $usersDetailed.Count -eq 0) {
    Write-Host "Raporlanacak kullanıcı bulunamadı. (Gruplar boş olabilir ya da hepsi bulunamamış olabilir.)" -ForegroundColor Yellow
    exit 0
}

# --- Excel'e yaz ---
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$sanitizedGroups = (($groupInputs -join "_") -replace '[\\/:*?"<>|]', '_')
if ($sanitizedGroups.Length -gt 80) { $sanitizedGroups = $sanitizedGroups.Substring(0,80) }

$outPath = Join-Path $env:TEMP "AD_GrupUyeleri_${sanitizedGroups}_${timestamp}.xlsx"

$usersDetailed |
    Sort-Object "Ad Soyad" |
    Export-Excel -Path $outPath `
        -WorksheetName "Grup Üyeleri" `
        -TableName "GrupUyeleri" `
        -AutoSize -AutoFilter -FreezeTopRow -BoldTopRow -ClearSheet

Write-Host "Excel oluşturuldu: $outPath" -ForegroundColor Green

# --- Otomatik aç ---
try {
    Invoke-Item -Path $outPath
} catch {
    Write-Host "Dosya oluşturuldu ancak otomatik açılamadı: $outPath" -ForegroundColor Yellow
}
